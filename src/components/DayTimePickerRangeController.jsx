...
export default class DayTimePickerRangeController extends React.Component {
    constructor(props) {
        super(props);
    ...
    onDayClick(selDay, e) {
        ...
        let day;
        if (focusedInput === START_DATE) { day = selDay.clone().set({ hour: this.state.startTime.hour(), minute: this.state.startTime.minute() }) }
        else if (focusedInput === END_DATE) { day = selDay.clone().set({ hour: this.state.endTime.hour(), minute: this.state.endTime.minute() }) }
